...
- `storage_mode="partitioned"` is refused as a configuration error (`absurd.E015`), and
...
| `absurd.E012` | The central `cron.database_name` database is unreachable or missing the `pg_cron` extension — a deploy-time check, quiet under a test suite unless `PG_CRON_ON_TEST_DB` (see [Cron Jobs](cron-jobs.md#operator-setup)).                                            |
