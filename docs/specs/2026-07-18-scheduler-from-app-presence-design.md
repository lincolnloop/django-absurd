...
- `absurd.E008` (`check_scheduler_app_installed`'s error branch, `E008_MSG`,
  ...
  ordering whenever the app is installed regardless of scheduler value.
...
| `checks.py`                                        | `check_scheduler_app_installed`                   | drop the E008 error branch + `E008_MSG`/`E008_HINT`; keep W003           |
...
**Delete** (test now-impossible states): `tests/core/test_scheduler_app_checks.py`
(whole file — E008 only);
...
and `test_reconcile_emits_teardown_notice_when_backend_switches` (scheduler-switch-
while-installed — no longer reachable);
...
across both suites (`tests/pg_cron/utils.py::build_pg_cron_tasks`/ `build_beat_tasks`,
...
`test_admin/test_scheduledtask.py`, `test_cross_source_coexistence.py`,
`test_absurd_sync_crons_command.py`, `test_checks.py`); each suite's INSTALLED_APPS now
fully determines the scheduler, so no test should set the key at all.
`tests/pg_cron/test_scheduler_app_checks.py` — drop the E008-not-in-out assertions, keep
the W003 ordering cases.
