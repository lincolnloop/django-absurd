...
CREATE VIEW "absurd"."checkpoints_view" AS SELECT NULL::text AS queue, NULL::text AS natural_key, NULL::uuid AS "task_id", NULL::text AS "checkpoint_name", NULL::jsonb AS "state", NULL::text AS "status", NULL::uuid AS "owner_run_id", NULL::timestamptz AS "updated_at" WHERE false;

DROP VIEW IF EXISTS "absurd"."events_view";
CREATE VIEW "absurd"."events_view" AS SELECT NULL::text AS queue, NULL::text AS natural_key, NULL::text AS "event_name", NULL::jsonb AS "payload", NULL::timestamptz AS "emitted_at" WHERE false;

DROP VIEW IF EXISTS "absurd"."waits_view";
CREATE VIEW "absurd"."waits_view" AS SELECT NULL::text AS queue, NULL::text AS natural_key, NULL::uuid AS "task_id", NULL::uuid AS "run_id", NULL::text AS "step_name", NULL::text AS "event_name", NULL::timestamptz AS "timeout_at", NULL::timestamptz AS "created_at" WHERE false;
"""
