...
def test_barrier_repeats_each_arm_and_reports_the_median() -> None:
    payload = run_barrier(queue="alpha", fast=6, slow=2, concurrency=2, repeat=2)

    entry = payload["entries"][0]
    assert len(entry["runs"]) == 2
    assert entry["spread"]["span_s"]["min"] <= entry["span_s"]
