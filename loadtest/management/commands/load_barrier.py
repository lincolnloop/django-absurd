...
class Command(BaseCommand):
            ...
            for distribution in DISTRIBUTIONS:
                for arm in ARMS:
                    entry = results.summarize_repeats(
                        functools.partial(
                            self.measure_arm, probe, workload, distribution, arm
                        ),
                        repeat=options["repeat"],
                        rank_by="span_s",
                        metrics=("tasks_per_sec", "utilization", "idle_slot_s"),
                    )
    ...
    def measure_arm(
        ...
        label = f"{workload} {distribution} {topology.arm}"
        with clocks.measure_phase(label) as phase:
    ...
    def report(
        ...
        self.stdout.write(
            f"\n{'workload':<9}{'distribution':<14}{'topology':<10}{'batch':>7}"
            f"{'span_s':>9}"
            f"{'tasks_per_sec':>15}{'mean_busy':>11}{'utilization':>13}"
            f"{'idle_slot_s':>13}{'idle_share':>12}{'runs':>6}{'spread':>20}"
        )
        for entry in entries:
            self.stdout.write(
                f"{entry['workload']:<9}{entry['distribution']:<14}"
                ...
                f"{entry['idle_share']:>12}{len(entry['runs']):>6}"
                f"{results.format_spread(entry, 'span_s'):>20}"
            )
        self.stdout.write(
            f"\n{'workload':<9}{'distribution':<14}{'pooled_s':>10}{'split_s':>9}"
            f"{'ratio':>7}{'pooled_idle_slot_s':>20}{'split_idle_slot_s':>19}"
        )
        for comparison in comparisons:
            self.stdout.write(
                f"{comparison['workload']:<9}{comparison['distribution']:<14}"
...
def start_resident_workers(
    ...
    draining: a draining worker runs django-absurd's own drain loop, and the barrier
    under test is the SDK's blocking one.
    """
