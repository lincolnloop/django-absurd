...
Three phases per queue: template (real ``enqueue`` + a draining worker, so every row is
...
from django_absurd.worker import drain_queue
from loadtest import preflight, schema, tasks
...
# Zero backoff so the same drain exhausts every attempt: the template then carries a
...
class Command(BaseCommand):
    ...
    def drain(self, queue: str) -> None:
        # The template phase only needs the backlog gone, at any concurrency, so the
        # plain public drain suffices here — unlike load_drain, which measures a
        # stated concurrency and has to reach for arun_drain.
        drain_queue(queue)
