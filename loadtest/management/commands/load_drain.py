...
and waits for every one of them to exit — ``--burst`` drains and returns on its own.

``elapsed_s`` is wall clock around that whole worker phase, so it includes each child's
interpreter start-up, ``django.setup()`` and queue provisioning — a fixed cost of
...
class Command(BaseCommand):
    ...
    def handle(self, *args: t.Any, **options: t.Any) -> str:
        ...
        for cell in cells:
            for workload in workloads:
                entry = self.drain_cell(
                    cell, workload, backlog=options["tasks"], timeout=options["timeout"]
                )
    ...
    def drain_cell(
        ...
        run_burst_workers(cell, timeout=timeout)
        ...
        if distinct_tasks != backlog:
            msg = (
                ...
                "record for it. A task left retrying on a backoff outlives a burst "
...
def run_burst_workers(cell: Cell, *, timeout: float) -> None:
    environ = build_worker_environ()
    context = multiprocessing.get_context("spawn")
    children = [
        context.Process(
            target=manage.run_burst_worker,
