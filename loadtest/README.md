...
- how the [admin](../docs/WHY.md#admin--orm-introspection) behaves once the entity views
  span millions of task rows,
...
`load_seed` enqueues a handful of real tasks per queue and drains them with a worker
that exits once the backlog is empty, so every seeded row is one Absurd itself wrote —
completed, failed with a retry history, still pending, and one suspended
[workflow](../docs/web/workflows.md) leaving a checkpoint, an event and a wait behind.
It then clones those templates server-side in 100k chunks until the queue holds
`--tasks` rows, jittering every timestamp across `--window` days (default 30), and
`ANALYZE`s each table it touched.
...
Every probe measures each arm **three times** and reports the **median run** — one real
observation, not a field-by-field blend, so an arm's numbers still describe the same
timeline as each other. `--repeat N` changes the count and `--repeat 1` takes a single
draw, which is fast and unsafe to quote. The spread (`min-max (cv%)`) sits beside the
number and every run is kept under `runs` in the JSON. This is not ceremony:
`load_drain`'s 4x4 async cell has been measured from 332 to 976 tasks/sec on one machine
in one sitting, and a single-shot probe reports whichever end it landed on.

Every run's JSON also carries an `environment` block — interpreter, Django, absurd-sdk,
schema version, arch, the Postgres version string, `shared_buffers`,
`effective_cache_size`, `max_parallel_workers_per_gather` and the checkout's HEAD. Two
dates' numbers cannot be compared without it.

`load_admin` discards one untimed request per arm before it starts timing, so all its
numbers are **warm**. Whichever arm hits a table first pays that table's cold read, and
with the arms in a fixed order that cost always landed on the same one — which is how an
unfiltered changelist came to be recorded at 85 s when the warm cost is nearer 1.6 s.
Cold-cache behaviour is deliberately **not** measured here: it needs a cache-drop
control the harness does not have. The `EXPLAIN` dumps are taken after the timed
requests and are therefore warm too, which the JSON records as `plan_regime`.

Every measured phase is bracketed by **both** clocks and refused if they disagree.
`time.perf_counter()` is `mach_absolute_time` on macOS and does not advance while the
machine is suspended, so a laptop that sleeps mid-arm reports a short and entirely
plausible elapsed time for a phase that really took minutes — while the tasks caught in
flight record wall-clock intervals spanning the whole nap, and any claim that outlived
its timeout is redelivered. One such arm reported `span_s` 209 against `elapsed_s` 11
with 422 rows for a 420-task backlog, and every individual field looked fine. Detection
rather than prevention: `caffeinate -is` is worth holding over a long run, but an
assertion that quietly lapses leaves no trace and a refused arm does.

Every probe refuses to run against a database with unapplied migrations. The load
database is persistent and nobody's test database, so nothing re-migrates it — it sat a
whole Absurd release behind for three weeks while the probes cheerfully reported numbers
for a schema the checkout no longer shipped.
