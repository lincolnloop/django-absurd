...
| `docs/web/` (Zensical site)        | **end users** (public docs site)               | The public **documentation site**: `docs/web/*.md` → built to `site/` (PR #30 / GitHub Pages). Navigable pages — **Home / Tasks / Workflows / Cron Jobs / Cleanup / Logging / Testing / Configuration / How it works** — presenting AGENTS.md's material for humans (may add examples/links; must not contradict it). On a user-facing change, update the relevant page **and** the `nav` in `zensical.toml`. |
...
3. **docs/web/ (site)** — update the matching page (`tasks.md` / `workflows.md` /
   `cron-jobs.md` / `cleanup.md` / `logging.md` / `testing.md` / `configuration.md` /
   `how-it-works.md`, or `index.md` for the quickstart) so the site tracks AGENTS.md.
