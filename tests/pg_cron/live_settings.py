"""Settings module for test_sync_schedules_on_migrate.py's subprocess-based real-DB
test. Invoked via `python -m django migrate --settings=tests.pg_cron.live_settings`
in a genuinely separate process — its own fresh PgCronConfig.ready() never sees a
test-DB swap, so is_test_db correctly evaluates False there. Connection params and
